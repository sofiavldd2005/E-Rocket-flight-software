...
struct TopicHealth {
  ...
  void update(uint64_t timestamp_us) {
    ...
    if (inter_message_times_ms.size() >= 5) { // Need at least 5 samples
      ...
      if (actual_frequency_hz <
          min_expected_frequency_hz * 0.9) { // 90% of expected
        std::ostringstream oss;
        oss << "Frequency too low: " << std::fixed << std::setprecision(1)
            << actual_frequency_hz
            << " Hz (expected >= " << min_expected_frequency_hz << " Hz)";
        last_error_message = oss.str();
        is_healthy = false;
        return;
      }
...
public:
  /**
   * @brief Construct a new System Monitor node
   *
   * Initializes parameters for thresholds, sets up subscriptions to monitored
   * topics, and starts the health check timer.
   */
  explicit SystemMonitor() : Node("system_monitor") {
    ...
    min_expected_frequency_hz_ =
        get_parameter("min_expected_frequency_hz").as_int();
    ...
    RCLCPP_INFO(get_logger(), "  min_expected_frequency_hz: %d",
                min_expected_frequency_hz_);
    ...
    health_check_timer_ =
        create_wall_timer(std::chrono::milliseconds(check_interval_ms),
                          [this] { check_health_and_report(); });
  }

private:
  std::map<std::string, TopicHealth>
      topic_health_; ///< Map storing the health state of each monitored topic

  // Thresholds
  int max_message_age_ms_; ///< Maximum allowed age for a message before it's
                           ///< considered stale
  int min_expected_frequency_hz_; ///< Minimum expected publishing frequency for
                                  ///< topics
  int stale_topic_timeout_ms_;    ///< Timeout duration after which a topic is
                                  ///< declared fully stale

  rclcpp::TimerBase::SharedPtr
      health_check_timer_; ///< Timer triggering the periodic health check
  ...
  rclcpp::Subscription<px4_msgs::msg::ActuatorMotors>::SharedPtr
      actuator_motors_sub_;
  rclcpp::Subscription<px4_msgs::msg::ActuatorServos>::SharedPtr
      actuator_servos_sub_;
  rclcpp::Subscription<erocket::msg::FlightMode>::SharedPtr flight_mode_sub_;

  /**
   * @brief Initialize all ROS 2 subscriptions to the critical topics being
   * monitored.
   */
  void setup_subscriptions() {
    ...
    angular_velocity_sub_ =
        create_subscription<px4_msgs::msg::VehicleAngularVelocity>(
            ...
            [this](const px4_msgs::msg::VehicleAngularVelocity::SharedPtr msg) {
              topic_health_["/fmu/out/vehicle_angular_velocity"].update(
                  msg->timestamp);
            });
    ...
    local_position_sub_ =
        create_subscription<px4_msgs::msg::VehicleLocalPosition>(
            ...
            [this](const px4_msgs::msg::VehicleLocalPosition::SharedPtr msg) {
              topic_health_["/fmu/out/vehicle_local_position"].update(
                  msg->timestamp);
            });
    ...
    topic_health_["/fmu/out/vehicle_attitude"].topic_name =
        "/fmu/out/vehicle_attitude";
    topic_health_["/fmu/out/vehicle_attitude"].max_message_age_ms =
        max_message_age_ms_;
    topic_health_["/fmu/out/vehicle_attitude"].min_expected_frequency_hz =
        100; // PX4 publishes at ~100 Hz

    topic_health_["/fmu/out/vehicle_angular_velocity"].topic_name =
        "/fmu/out/vehicle_angular_velocity";
    topic_health_["/fmu/out/vehicle_angular_velocity"].max_message_age_ms =
        max_message_age_ms_;
    topic_health_["/fmu/out/vehicle_angular_velocity"]
        .min_expected_frequency_hz = 100;

    topic_health_["/fmu/out/vehicle_local_position"].topic_name =
        "/fmu/out/vehicle_local_position";
    topic_health_["/fmu/out/vehicle_local_position"].max_message_age_ms =
        max_message_age_ms_;
    topic_health_["/fmu/out/vehicle_local_position"].min_expected_frequency_hz =
        50;

    topic_health_["/fmu/out/vehicle_odometry"].topic_name =
        "/fmu/out/vehicle_odometry";
    topic_health_["/fmu/out/vehicle_odometry"].max_message_age_ms =
        max_message_age_ms_;
    topic_health_["/fmu/out/vehicle_odometry"].min_expected_frequency_hz = 50;

    topic_health_["/fmu/in/actuator_motors"].topic_name =
        "/fmu/in/actuator_motors";
    topic_health_["/fmu/in/actuator_motors"].max_message_age_ms =
        max_message_age_ms_;
    topic_health_["/fmu/in/actuator_motors"].min_expected_frequency_hz = 50;

    topic_health_["/fmu/in/actuator_servos"].topic_name =
        "/fmu/in/actuator_servos";
    topic_health_["/fmu/in/actuator_servos"].max_message_age_ms =
        max_message_age_ms_;
    topic_health_["/fmu/in/actuator_servos"].min_expected_frequency_hz = 50;

    topic_health_["offboard/flight_mode/get"].topic_name =
        "offboard/flight_mode/get";
    topic_health_["offboard/flight_mode/get"].max_message_age_ms =
        1000; // Less critical
    topic_health_["offboard/flight_mode/get"].min_expected_frequency_hz = 1;
  }

  /**
   * @brief Periodic callback to evaluate the health of all topics and log any
   * issues.
   */
  void check_health_and_report() {
    ...
    for (auto &[topic_name, health] : topic_health_) {
      ...
      if (health.message_count > 0 &&
          health.is_stale(std::chrono::milliseconds(stale_topic_timeout_ms_))) {
        RCLCPP_ERROR(
            get_logger(), "TOPIC STALE: %s (last message %.1f seconds ago)",
            topic_name.c_str(),
